...
@dataclass
class MediaMarkers:
    frame_rate_value: str
    time_format_value: str
    start_timecode_value: str
    start_time_scale: int
    start_time_sample_size: int
    markers: list
...
def _frame_rate_to_xmp(frame_duration: Fraction) -> str:
    if frame_duration <= 0:
        return "f0"
    fps = Fraction(1, 1) / frame_duration
    if fps.denominator == 1:
        return f"f{fps.numerator}"
    return f"f{fps.numerator}s{fps.denominator}"


def _frame_duration_to_scale_sample(frame_duration: Fraction) -> tuple[int, int]:
    if frame_duration <= 0:
        return 0, 1
    return frame_duration.denominator, frame_duration.numerator


def _detect_time_format(frame_duration: Fraction, tc_format: str) -> str:
    tc_kind = (tc_format or "NDF").upper()
    lookup = {
        Fraction(1001, 24000): ("23976Timecode", "23976Timecode"),
        Fraction(1, 24): ("24Timecode", "24Timecode"),
        Fraction(1, 25): ("25Timecode", "25Timecode"),
        Fraction(1001, 30000): ("2997NonDropTimecode", "2997DropTimecode"),
        Fraction(1, 30): ("30Timecode", "30Timecode"),
        Fraction(1, 50): ("50Timecode", "50Timecode"),
        Fraction(1001, 60000): ("5994NonDropTimecode", "5994DropTimecode"),
        Fraction(1, 60): ("60Timecode", "60Timecode"),
    }
    ndf_value, df_value = lookup.get(frame_duration, ("", ""))
    if tc_kind == "DF" and df_value:
        return df_value
    if ndf_value:
        return ndf_value
    return ""
...
def _frames_from_seconds(seconds: Fraction, frame_duration: Fraction) -> int:
    if frame_duration <= 0:
        return 0
    return _round_fraction_to_int(seconds / frame_duration)


def _format_timecode(frame_count: int, frame_duration: Fraction, tc_format: str) -> str:
    fps_map = {
        Fraction(1001, 24000): 24,
        Fraction(1, 24): 24,
        Fraction(1, 25): 25,
        Fraction(1001, 30000): 30,
        Fraction(1, 30): 30,
        Fraction(1, 50): 50,
        Fraction(1001, 60000): 60,
        Fraction(1, 60): 60,
    }
    nominal_fps = fps_map.get(frame_duration)
    if not nominal_fps or frame_count < 0:
        return "00:00:00:00"

    tc_kind = (tc_format or "NDF").upper()
    if tc_kind != "DF" or frame_duration not in (Fraction(1001, 30000), Fraction(1001, 60000)):
        frames = frame_count
    ...
    ten_minute_blocks = frames // frames_per_10_minutes
    frames %= frames_per_10_minutes

    minutes = ten_minute_blocks * 10
    while frames >= frames_per_minute:
        frames -= frames_per_minute
        minutes += 1
        if minutes % 10 == 0:
            frames_per_minute = frames_per_10_minutes
        else:
...
def parse_fcpxml(fcpxml_path: str, logger=None):
    ...
    for clip in root.findall(".//{*}clip"):
        ...
        tc_format = (clip.get("tcFormat") or "NDF").upper()
        ...
        frame_rate_value = _frame_rate_to_xmp(frame_duration)
        time_format_value = _detect_time_format(frame_duration, tc_format)
        clip_reports.append(
            ClipReport(
                clip_name=clip_name,
                ...
                time_format_value=time_format_value or "unknown",
            )
        )
        ...
        if bucket is None:
            ...
            asset_start_frames = _frames_from_seconds(asset_start_seconds, frame_duration)
            start_timecode_value = _format_timecode(asset_start_frames, frame_duration, tc_format)
            start_time_scale, start_time_sample_size = _frame_duration_to_scale_sample(frame_duration)
            bucket = MediaMarkers(
                frame_rate_value=frame_rate_value,
                time_format_value=time_format_value,
                start_timecode_value=start_timecode_value,
                start_time_scale=start_time_scale,
                start_time_sample_size=start_time_sample_size,
                markers=[],
            )
        ...
        for rating in clip.findall(".//{*}rating"):
            ...
            start_units = _frames_from_seconds(start_offset_seconds, frame_duration)
            duration_units = _frames_from_seconds(rating_duration_seconds, frame_duration)
            bucket.markers.append(Marker(start_units, duration_units, name, color))

    return markers_by_media, clip_reports


def build_premiere_xmp(media_markers: MediaMarkers) -> str:
    lines = [
    ...
    if media_markers.time_format_value:
        lines.extend(
            [
                "      <xmpDM:startTimecode rdf:parseType=\"Resource\">",
                f"        <xmpDM:timeValue>{media_markers.start_timecode_value}</xmpDM:timeValue>",
                f"        <xmpDM:timeFormat>{media_markers.time_format_value}</xmpDM:timeFormat>",
                "      </xmpDM:startTimecode>",
                "      <xmpDM:altTimecode rdf:parseType=\"Resource\">",
                f"        <xmpDM:timeValue>{media_markers.start_timecode_value}</xmpDM:timeValue>",
                f"        <xmpDM:timeFormat>{media_markers.time_format_value}</xmpDM:timeFormat>",
                "      </xmpDM:altTimecode>",
                f"      <xmpDM:startTimeScale>{media_markers.start_time_scale}</xmpDM:startTimeScale>",
                f"      <xmpDM:startTimeSampleSize>{media_markers.start_time_sample_size}</xmpDM:startTimeSampleSize>",
            ]
        )
    ...
    lines.extend(
        [
            ...
            f"            <xmpDM:frameRate>{media_markers.frame_rate_value}</xmpDM:frameRate>",
            "            <xmpDM:markers>",
            "              <rdf:Seq>",
        ]
    )

    for marker in media_markers.markers:
        guid = str(uuid.uuid4())
        color_guid = str(uuid.uuid4())

        lines.append("                <rdf:li rdf:parseType=\"Resource\">")
        lines.append(f"                  <xmpDM:startTime>{marker.start_units}</xmpDM:startTime>")
        if marker.duration_units > 0:
            lines.append(f"                  <xmpDM:duration>{marker.duration_units}</xmpDM:duration>")
        if marker.name:
...
def process_all(fcpxml_dir: str, media_dir: str, output_dir: str, exiftool_path: str, logger):
    ...
    for report in all_clip_reports:
        logger(
            f"  - {report.clip_name} -> {report.media_name} | "
            f"tc={report.tc_format} | frameRate={report.frame_rate_value} | "
            f"timeFormat={report.time_format_value}"
        )
    ...
    for media_key, media_markers in sorted(markers_by_media.items()):
        logger(
            f"  - {media_key}: {len(media_markers.markers)} markers | "
            f"frameRate={media_markers.frame_rate_value} | "
            f"timeFormat={media_markers.time_format_value or 'unknown'}"
        )
    ...
    for media_key, media_markers in markers_by_media.items():
        ...
        try:
            ...
            xmp_content = build_premiere_xmp(media_markers)
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("FCPXML to Premiere XMP (Native Timebase)")
    ...
    def _build_ui(self):
        ...
        self.log_text = scrolledtext.ScrolledText(self.root, width=100, height=22)
